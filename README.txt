 ...
 * Check and verify locally using CP access type
 	- horcrux-cli mount AMCC cp:///usr/share/horcrux/amcc /mnt (NOTE: full path after "cp://", so three slashes in total)
  ...
  - For containerized development, use horcrux-dv as a volume plugin
	...
	- Create docker volumes using horcrux volume plugin, and using the right access type option
	  ...
	  - with SCP
		...
		(uses passwd less access. If you want to provide passwd, use --access=scp://<user>::<passwd>@<host>:/<full-path> NOTE: not secure :)
 ...
 * Bazil (github.com/bazil) folks for their Go FUSE on top of which this is built.
